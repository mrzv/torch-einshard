...
def test_reducescatter_forward_allgather_backward(dist_env, mesh_1d):
    group = mesh_1d["dp"].get_group()
    rank = dist.get_rank(group)
    world_size = dist.get_world_size(group)
    rows = world_size * 2
    ...
    z = reducescatter_forward_allgather_backward(x, group, 0, shapes)
...
def test_allgather_forward_reducescatter_backward(dist_env, mesh_1d):
    group = mesh_1d["dp"].get_group()
    rank = dist.get_rank(group)
    world_size = dist.get_world_size(group)
    rows = world_size * 2
    ...
    full = torch.arange(rows * 3, dtype=torch.float32).reshape(rows, 3)
    x = torch.split(full, shapes, dim=0)[rank].clone().requires_grad_(True)
    z = allgather_forward_reducescatter_backward(x, group, 0, shapes)
